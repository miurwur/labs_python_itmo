...
Группа: P3122  
Ссылка на colab с выполнением работы: https://colab.research.google.com/drive/1GSYJdAxqrWp1idRrvepkQ-6KeoH48DYe#scrollTo=63phKiaTQcWt  
...
1. **Многопоточность** с включенным GIL неэффективна для CPU-задач из-за принудительной последовательности выполнения Python-кода.

2. **Многопроцессность** обеспечивает ускорение для CPU-задач за счет параллелизма, но имеет значительные накладные расходы на создание процессов.

3. **Cython** даёт максимальное ускорение благодаря компиляции в машинный код и устранению накладных расходов интерпретатора.

4. **NOGIL в Cython** позволяет обойти ограничения GIL, делая многопоточность эффективной для вычислительных задач.
